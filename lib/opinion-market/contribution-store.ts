...
export function useOpinionContributions(matchId: string, momentId?: string) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (client) {
      ...
      if (!error) { setRows((data ?? []).map((row) => ({ id: row.id, matchId: row.match_id, momentId: row.moment_id, amountSol: Number(row.amount_lamports) / 1_000_000_000, signature: row.transaction_signature, mode: row.mode, createdAt: row.created_at }))); return; }
    }
