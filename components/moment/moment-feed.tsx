...
export function MomentFeed({ moments }: { moments: MomentView[] }) {
  ...
  const [selectedMoment, setSelectedMoment] = useState<MomentView | null>(null);
  ...
  return <>
    ...
    <motion.div layout className="moment-grid">{sorted.map((moment, index) => <motion.div layout key={moment.id}><MomentCard moment={moment} featured={index === 0 && sort === "top"} onOpen={() => setSelectedMoment(moment)} /></motion.div>)}</motion.div>
    <AnimatePresence>{selectedMoment && <MomentDetail moment={selectedMoment} onClose={() => setSelectedMoment(null)} />}</AnimatePresence>
  </>;
}
