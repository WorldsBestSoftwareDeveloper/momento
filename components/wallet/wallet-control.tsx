...
export function WalletControl({ compact = false }: { compact?: boolean }) {
  ...
  if (!connected) return <button type="button" className={compact ? "wallet-connect compact" : "wallet-connect"} onClick={() => setVisible(true)} disabled={connecting}><Wallet size={17} />{connecting ? "Connecting…" : "Connect Wallet"}</button>;

  const copy = async () => {
    await navigator.clipboard.writeText(address); setCopied(true); window.setTimeout(() => setCopied(false), 1_500);
  };

  return <div className={`wallet-control ${compact ? "is-compact" : ""}`}>
    <button type="button" className="wallet-connected-button" onClick={() => setOpen((value) => !value)} aria-expanded={open}>
      ...
      <span><small>Connected • Devnet</small><strong>{shortWalletAddress(address)}</strong></span><ChevronDown size={15} />
    </button>
