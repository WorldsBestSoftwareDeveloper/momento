...
export function RewardsDashboard({ reward }: { reward: CreatorReward }) {
  ...
  const { mode } = useCanonicalMatchState();
  return <div className="rewards-layout">
    <section className="rewards-hero"><div><span className="eyebrow"><Trophy size={14} /> Creator rewards</span><h1>Great reactions deserve a victory lap.</h1><p>Momento rewards the creator behind the match’s Winning Moment. Browsing, uploading, commenting and Champion actions never require a wallet.</p></div>{mode === "live" && <WalletControl />}</section>
    <div className="rewards-main-grid"><CreatorRewardCard reward={reward} claimingAvailable={mode === "live"} onClaimed={() => void state.refresh()} />
      <aside className="reward-facts">
        ...
        <dl><div><dt>Reward amount</dt><dd>{reward.amountSol} SOL</dd></div><div><dt>Network</dt><dd>Solana Devnet</dd></div><div><dt>Wallet</dt><dd>{mode === "replay" ? "Not required" : state.address ? shortWalletAddress(state.address) : "Not connected"}</dd></div><div><dt>Claim status</dt><dd>{state.claim ? "Claimed" : mode === "replay" ? "Available after live settlement" : "Treasury signer required"}</dd></div><div><dt>Latest claim</dt><dd>{state.claim ? new Date(state.claim.claimedAt).toLocaleDateString() : "—"}</dd></div></dl>
      </aside>
    </div>
