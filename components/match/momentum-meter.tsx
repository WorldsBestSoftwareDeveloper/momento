...
export function MomentumMeter({ match }: { match: MatchRoomView }) {
  ...
  return (
    <aside className="momentum-panel">
      ...
      <p><strong>{leader?.title ?? "Awaiting the first Moment"}</strong><br />{leader ? "Current leader" : "Live rankings begin with the first reaction"}</p>
