...
export function ReplayControls(props: ReplayControlsProps) {
  ...
  return (
    <section className="replay-controls" aria-label="Demo replay controls">
      <div><span className="eyebrow">Replay controller</span><strong>{beatLabel}</strong><small>Beat {Math.max(cursor + 1, 0)} of {total}</small></div>
      <div className="replay-control-actions">
        ...
        <button type="button" onClick={onNext} disabled={completed || cursor < 0}><SkipForward size={15} /> Next beat</button>
        <button type="button" onClick={onFinish} disabled={completed || cursor < 0}>Final</button>
      </div>
    </section>
  );
}
